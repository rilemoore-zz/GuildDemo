...
namespace App2
{
    ...
    public partial class Page29 : ContentPage, IAnimationPage
    {
        public Page29()
        {
            InitializeComponent();
       ...
       // public List<GameEvent> events = new List<GameEvent>();
        async void GetEvents()
        {
           // events = await App.RestService.GetResponse<List<GameEvent>>(Constants.LoginUrl);
            Title.Text = Constants.events[9].EventTitle;
            StartDate.Text = Constants.events[9].StartDate.ToString();
            StartTime.Text = Constants.events[9].StartTime.ToString();
            //UserId.Text = events[9].UserId.ToString();
            Game.Text = Constants.events[9].EventGame;
            Platform.Text = Constants.events[9].Platform.ToString();
            // NumPlayers.Text = events[9].numberOfPlayers.ToString();

        }

        void Handle_Clicked(object sender, System.EventArgs e)
        {
            if (Constants.RegisteredEvents.Contains(Constants.events[9]))
            {
                DisplayAlert("Error", "Event Already Registered", "Ok");
            }
            else
            {
                Constants.RegisteredEvents.Add(Constants.events[9]);
            }
        ...
        async void Del(object sender, System.EventArgs e)
        {
            ...
            if (Constants.CurrentUser.ID == Constants.events[9].UserId)
            {
                string myPostedEvent = JsonConvert.SerializeObject(Constants.events[9]);
                ...
                Constants.events.Remove(Constants.events[9]);
            }
        ...
        async void Inv(object sender, System.EventArgs e)
        {
            if (Constants.CurrentUser.ID == Constants.events[9].UserId)
            {
                ...
                newattendee.EventID = Constants.events[9].eventId;
