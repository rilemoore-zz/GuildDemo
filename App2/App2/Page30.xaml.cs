    ...
    public partial class Page30 : ContentPage, IAnimationPage
    {
       ...
       // public List<GameEvent> events = new List<GameEvent>();
        async void GetEvents()
        {
            ...
            Title.Text = Constants.events[10].EventTitle;
            StartDate.Text = Constants.events[10].StartDate.ToString();
            StartTime.Text = Constants.events[10].StartTime.ToString();
            //UserId.Text = events[10].UserId.ToString();
            Game.Text = Constants.events[10].EventGame;
            Platform.Text = Constants.events[10].Platform.ToString();
        ...
        {
            if (Constants.RegisteredEvents.Contains(Constants.events[10]))
            {
                DisplayAlert("Error", "Event Already Registered", "Ok");
            }
            else
            {
                Constants.RegisteredEvents.Add(Constants.events[10]);
            }
        ...
        async void Del(object sender, System.EventArgs e)
        {
            ...
            if (Constants.CurrentUser.ID == Constants.events[10].UserId)
            {
                string myPostedEvent = JsonConvert.SerializeObject(Constants.events[10]);
                ...
                Constants.events.Remove(Constants.events[10]);
            }
        ...
        async void Inv(object sender, System.EventArgs e)
        {
            if (Constants.CurrentUser.ID == Constants.events[10].UserId)
            {
                ...
                newattendee.EventID = Constants.events[10].eventId;
